...
import { ClaimRefFeeOption, CreatePartnerOption, Partner, UpdateRefFeeRateOption, UpdateTimeRangeOption } from '../types/clmm_type'
import { BASIS_POINT, parsePartner } from '../utils/common'
import { handleError, PartnerErrorCode } from '../errors/errors'

export class PartnerModule implements IModule<CetusClmmV2SDK> {
  ...
  async getPartnerList(): Promise<Partner[]> {
    ...
    const { partners_id } = getPackagerConfigs(clmm_pool)
    try {
      ...
      return handleError(PartnerErrorCode.FetchError, error as Error, {
  ...
  async getPartnerCapId(owner: string, partner_id: string): Promise<string> {
    ...
    try {
      ...
      const res = await this._sdk.FullClient.getOwnedObjects({
        ...
        filter: {
          StructType: `${clmm_pool.package_id}::partner::PartnerCap`,
        },
      })
      ...
      if (!partnerCapId) {
        return handleError(PartnerErrorCode.NotFoundPartnerObject, new Error('Partner cap not found'), {
  ...
  updateRefFeeRatePayload(option: UpdateRefFeeRateOption, tx?: Transaction): Transaction {
    ...
    const ref_fee_rate_raw = d(ref_fee_rate).mul(1)
    if (ref_fee_rate_raw.gt(BASIS_POINT)) {
      return handleError(PartnerErrorCode.InvalidParams, new Error('ref_fee_rate is cannot be greater than 1'), {
    ...
    tx.moveCall({
      target: `${clmm_pool.published_at}::partner::update_ref_fee_rate`,
  ...
  async claimRefFeePayload(option: ClaimRefFeeOption) {
    ...
    fee_coin_types.forEach((coin_type) => {
      tx.moveCall({
        target: `${clmm_pool.published_at}::partner::claim_ref_fee`,
  ...
  updateTimeRangePayload(option: UpdateTimeRangeOption, tx?: Transaction): Transaction {
    ...
    if (endTimeInSeconds <= startTimeInSeconds) {
      return handleError(PartnerErrorCode.InvalidParams, new Error('end_time must be greater than start_time'), {
    ...
    tx.moveCall({
      target: `${clmm_pool.published_at}::partner::update_time_range`,
      arguments: [
  ...
  createPartnerPayload(option: CreatePartnerOption): Transaction {
    ...
    const { partners_id, global_config_id, versioned_id } = getPackagerConfigs(clmm_pool)
    ...
    tx.moveCall({
      target: `${clmm_pool.published_at}::partner::create_partner`,
      arguments: [
