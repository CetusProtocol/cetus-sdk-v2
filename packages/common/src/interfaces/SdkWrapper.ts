...
import { JsonRpcTransport, SuiJsonRpcClient, SuiJsonRpcClientOptions } from '@mysten/sui/jsonRpc'
import { url } from 'inspector'

export const buildSuiJsonRpcClient: (full_rpc_url: string, network: 'mainnet' | 'testnet') => SuiJsonRpcClient = (full_rpc_url: string, network: 'mainnet' | 'testnet') => new SuiJsonRpcClient({ url: full_rpc_url, network: network } as SuiJsonRpcClientOptions)
export const buildSuiGrpcClient: (full_rpc_url: string, network: 'mainnet' | 'testnet') => SuiGrpcClient = (full_rpc_url: string, network: 'mainnet' | 'testnet') => new SuiGrpcClient({ baseUrl: full_rpc_url, network: network })
export const buildSuiGraphQLClient: (graph_rpc_url: string, network: 'mainnet' | 'testnet') => SuiGraphQLClient = (graph_rpc_url: string, network: 'mainnet' | 'testnet') => new SuiGraphQLClient({ network: network, url: graph_rpc_url })
export abstract class SdkWrapper<T extends BaseSdkOptions> {
  ...
  private _jsonRpcClient: SuiJsonRpcClient | undefined
  ...
  constructor(options: T) {
    ...
    const resolvedGrpcClient = sui_grpc_client ? sui_grpc_client : full_rpc_url ? new SuiGrpcClient({ baseUrl: full_rpc_url, network: env }) : undefined

    const suiClient = sui_client ? sui_client : resolvedGrpcClient
    this._graphClient = graph_client
      ? graph_client
      : graph_rpc_url
        ? buildSuiGraphQLClient(graph_rpc_url, env)
        : undefined

    const jsonRpcClient = buildSuiJsonRpcClient(this._sdkOptions.full_rpc_url!, this._env);

    if (!suiClient) {
      throw new Error('sui_client or full_rpc_url is required')
    }

    this._fullClient = createFullClient(suiClient, this._graphClient, env, jsonRpcClient)
  }
  ...
  updateFullRpcUrl(full_rpc_url: string): void {
    this._sdkOptions.full_rpc_url = full_rpc_url
    this._fullClient = createFullClient(buildSuiGrpcClient(full_rpc_url, this._env), this._graphClient, this._env, buildSuiJsonRpcClient(full_rpc_url, this._env))
  }

  updateSuiClient(sui_client: SuiGrpcClient): void {
    this._fullClient = createFullClient(sui_client, this._graphClient, this._env, this._jsonRpcClient)
  }

  updateGraphRpcUrl(graph_rpc_url: string): void {
    this._sdkOptions.graph_rpc_url = graph_rpc_url
    this._graphClient = buildSuiGraphQLClient(graph_rpc_url, this._env)
    this._fullClient = createFullClient(this._fullClient._client, this._graphClient, this._env, this._jsonRpcClient)
  }
